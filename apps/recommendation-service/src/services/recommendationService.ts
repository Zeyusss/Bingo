...
async function fetchUserActivity(userId:string):Promise<UserActions[]>{
    const userActions = await getUserActivity(userId);
    return Array.isArray(userActions) ? (userActions as unknown as UserActions[]) : [];
}

export const recommendProducts = async (userId:string,allProducts:any): Promise<string[]> =>{
    const userActions:UserActions[] = await fetchUserActivity(userId);
    ...
    // create embedding layer -like look up tables- to learn the relationships 
    ...
    //convert user and product interactions into tensors for trainig
    const userTensor = tf.tensor1d(interactions.map((d)=> userMap[d.userId] ?? 0),
"int32");
