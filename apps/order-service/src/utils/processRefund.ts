...
export const processRefund = async (order: {
  ...
  if (method === 'paymob') {
    ...
    return { refunded: false, message: 'Paymob refund not yet implemented — process manually.' };
  }

  // Default: Stripe
  if (!order.paymentIntentId) {
    return { refunded: false, message: 'No paymentIntentId on record — process refund manually in Stripe dashboard.' };
  }
