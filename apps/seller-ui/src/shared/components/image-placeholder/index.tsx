...
const ImagePlaceHolder = ({size,small,onImageChange,onRemove,defaultImage = null , index= null,setOpenImageModal}:{
    ...
    {imagePreview ? (<Image width={400} height={300} src={imagePreview} alt="uploaded" className='w-full h-full object-cover rounded-lg'/>) : ( <><p className={`text-white ${small ? "text-xl" : "text-4xl"} font-semibold`}>{size}</p>
    <p className={`text-white ${small ? "text-sm" : "text-lg"} pt-2 text-center`}>Please choose an image <br/> according to the expected ratio</p>
    </>)}
    </div>
    )
}
