...
export default function SellerNotificationsPage() {
  ...
  const deleteAllUnreadMutation = useMutation({
    mutationFn: async () => {
      await enhancedAxiosInstance.delete("/seller/api/notifications/bulk-delete-unread");
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["seller-notifications"] });
  ...
  const handleDeleteAllUnread = () => {
    ...
    if (unreadCount === 0) {
      alert("No unread notifications to delete.");
      return;
    }
    if (confirm(`Are you sure you want to delete all ${unreadCount} unread notifications? This action cannot be undone.`)) {
      deleteAllUnreadMutation.mutate();
    }
  };
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={deleteAllUnreadMutation.isPending}
                className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                <Trash2 className="w-4 h-4" />
                {deleteAllUnreadMutation.isPending ? "Deleting..." : "Delete All Unread"}
              </button>
            </div>
          </div>
        </div>
