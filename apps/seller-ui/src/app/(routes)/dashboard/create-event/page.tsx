  ...
  const { data: allProducts = [], isLoading: isLoadingProducts } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axiosInstance.get(
        `/product/api/get-shop-products?sellerId=${seller.id}`
      );

      return (
        res.data.products?.filter(
          (product: Product) => !product.starting_date && product.stock < 100
        ) || []
      );
    },
  ...
  const createEventMutation = useMutation({
    mutationFn: async (eventData: CreateEventForm) => {
      ...
      const res = await axiosInstance.post(
        `/seller/api/create-event/${seller.shop.id}`,
        eventData
      );
      return res.data;
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["seller-products-for-events"],
      });
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-600">
            Turn your products into limited-time offers and events
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <p className="text-slate-600 text-sm mb-4">
            Choose a product with limited stock to create an event
          </p>
          ...
          {isLoadingProducts ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-slate-600 font-medium">
                No products with limited stock found
              </p>
              <p className="text-slate-500 text-sm mt-1">
                Products with less than 100 items in stock will appear here
              </p>
            </div>
