...
const CRON_JOBS_CONFIG = [
  ...
  },
  {
    ...
    description:
      "Send follow-up reminder emails every 3 days for abandoned carts",
    schedule: "0 14 */3 * *",
  ...
  },
  {
    ...
    description:
      "Deep cleanup of carts, cart items, and orphaned records older than 30 days",
    schedule: "0 4 1 * *",
  ...
  },
  {
    ...
    description:
      "Permanently delete products marked for deletion after grace period",
    schedule: "0 * * * *",
  ...
  },
  {
    ...
    description:
      "Process shop deletions and block associated sellers after grace period",
    schedule: "0 * * * *",
    frequency: "Every hour",
  ...
  try {
    ...
    switch (jobId) {
      ...
      case "abandoned_cart_followup_reminder": {
        ...
        while (
          lastFollowup > now ||
          (now.getTime() - lastFollowup.getTime()) %
            (3 * 24 * 60 * 60 * 1000) !==
            0
        ) {
          lastFollowup.setUTCDate(lastFollowup.getUTCDate() - 1);
          if (lastFollowup.getTime() < now.getTime() - 7 * 24 * 60 * 60 * 1000)
            break;
        }
...
export const getAllCronJobs = async (req: Request, res: Response) => {
  try {
    ...
    if (status && status !== "all") {
      finalJobs = jobsWithStatus.filter(
        (job) => job.status.toLowerCase() === (status as string).toLowerCase(),
      );
    }
...
export const getCronJobCategories = async (req: Request, res: Response) => {
  try {
    const categories = [
      ...new Set(CRON_JOBS_CONFIG.map((job) => job.category)),
    ];
...
export const getCronJobsStats = async (req: Request, res: Response) => {
  try {
    ...
    const stats = {
      ...
      successful: jobsWithStatus.filter((job) => job.status === "Success")
        .length,
      ...
      categories: {
        "Email Marketing": jobsWithStatus.filter(
          (job) => job.category === "Email Marketing",
        ).length,
        "Database Maintenance": jobsWithStatus.filter(
          (job) => job.category === "Database Maintenance",
        ).length,
        "Account Management": jobsWithStatus.filter(
          (job) => job.category === "Account Management",
        ).length,
      },
    };
