...
export const checkOtpRestrictions = async (email: string) => {
  if (await redis.get(`otp_lock:${email}`)) {
    throw new ValidationError(
      "You have reached the maximum number of OTP requests. Please try again after 30min.",
    );
  }
  if (await redis.get(`otp_spam_lock:${email}`)) {
    throw new ValidationError(
      "You are sending OTP requests too frequently. Please try again after 1 hour.",
    );
  }
  if (await redis.get(`otp_cooldown:${email}`)) {
    throw new ValidationError(
      "You have already requested an OTP. Please wait for 1 minute before requesting again.",
    );
  }
};
...
export const verifyUserRegistrationOtp = async (email: string, otp: string) => {
  ...
  if (cachedOtp !== otp) {
    if (failedAttempts >= 2) {
      ...
      throw new ValidationError(
        "You have exceeded the maximum number of OTP attempts. Please try again after 30 minutes.",
      );
    }
